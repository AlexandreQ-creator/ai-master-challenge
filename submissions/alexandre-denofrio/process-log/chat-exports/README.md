...
| `15-saida-harness-servidor-dashboard.txt` | Saída de `python solution/harness_servidor_dashboard.py` — 4 testes que sobem o processo real e batem via HTTP, incluindo confirmação por `mtime` de que o `dashboard.html` foi de fato regravado (não só que a chamada retornou sucesso), depois de corrigir o bug de `webbrowser.open()` travando o processo sob teste | Workflow, passo 24 |
