...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .chat-box {{
    ...
    gap: 10px; padding: 4px 2px 10px; margin-bottom: 12px;
  }}
  .chat-msg {{ max-width: 85%; padding: 10px 13px; border-radius: 10px; font-size: 13.5px; line-height: 1.55; }}
  .chat-msg.chat-user {{ align-self: flex-end; background: var(--ink); color: #fff; }}
  .chat-msg.chat-bot {{ align-self: flex-start; background: #f1f2f5; color: var(--ink); }}
  .chat-msg.chat-bot em {{ color: var(--muted); font-style: italic; }}
  .chat-msg.chat-bot ul {{ margin: 6px 0 0; padding-left: 18px; }}
  .chat-msg.chat-bot li {{ margin-bottom: 4px; }}
  .chat-msg.chat-erro {{ align-self: flex-start; background: var(--accent-soft); color: var(--accent); }}
  .chat-input-row {{ display: flex; gap: 8px; }}
  .chat-input-row input {{
    flex: 1; border: 1px solid var(--line); border-radius: 8px; padding: 9px 12px;
    font-size: 13.5px; font-family: inherit;
  }}
  .chat-input-row input:disabled {{ background: #f8f9fb; }}
  .chat-input-row button {{
    background: var(--ink); color: #fff; border: none; border-radius: 8px;
    padding: 9px 16px; font-size: 13px; font-weight: 600; cursor: pointer;
  }}
  .chat-input-row button:disabled {{ background: #c7cad1; cursor: not-allowed; }}
</style>
</head>
<body>
<div class="wrap">
  ...
  <section id="chatSection">
    <h2>Pergunte à IA de consulta</h2>
    <p class="desc" id="chatHint">
      Responde só sobre o que já foi analisado (causa raiz, segmentos, contas específicas, recomendações, metodologia, conformidade da submissão) — arquitetura de agentes especializados em <code>solution/arquitetura-ia-consulta.md</code>.
    </p>
    <div id="chatBox" class="chat-box">
      <div class="chat-msg chat-bot">Pergunte algo, por exemplo: <em>"por que o churn está subindo?"</em>, <em>"qual o MRR da Company_4?"</em>, <em>"o que a empresa deveria fazer?"</em></div>
    </div>
    <div class="chat-input-row">
      <input type="text" id="chatInput" placeholder="Pergunte sobre o diagnóstico..." maxlength="500" onkeydown="if(event.key==='Enter')enviarPergunta()">
      <button id="btnPerguntar" onclick="enviarPergunta()">Perguntar</button>
    </div>
  </section>
...
// Chat da IA de consulta: mesma restrição do botão "Atualizar dados" —
// só funciona via servidor_dashboard.py (POST /api/perguntar chama
// IAConsultaRavenStack().perguntar() por trás). Um arquivo file:// não
// tem como rodar Python no disco, então o chat fica desabilitado com uma
// explicação em vez de tentar e falhar em silêncio.
(function initChat() {{
  ...
  if (!servidoPorHttp) {{
    input.disabled = true;
    btn.disabled = true;
    input.placeholder = 'Desabilitado neste modo (file://)';
    hint.innerHTML = 'Desabilitado neste modo (<code>file://</code>). Rode <code>python solution/servidor_dashboard.py</code> para habilitar, ou pergunte direto no terminal com <code>python solution/ia_consulta.py "sua pergunta"</code>.';
  }}
}})();

// Parser mínimo do formato de resposta da IA (linhas "- item", "_itálico_",
// quebra dupla de linha) -- sem biblioteca de markdown, mesma filosofia de
// dependência mínima do resto da submissão. Escapa HTML antes de formatar,
// para nunca injetar o que a IA (ou, no limite, o usuário) escreveu como
// tag real -- toda entrada de rede aqui é tratada como texto, não HTML.
function formatarRespostaIA(texto) {{
  ...
  for (const linha of linhas) {{
    if (linha.startsWith('- ')) {{
      ...
      if (linha.trim()) html += '<p style="margin:6px 0;">' + linha + '</p>';
    }}
  }}
  if (dentroDeLista) html += '</ul>';
  // itálico "_texto_" -> <em>, só depois da lista pronta (evita conflito com "-")
...
async function enviarPergunta() {{
  ...
  msgUser.className = 'chat-msg chat-user';
  ...
  try {{
    ...
    if (resp.ok) {{
      msgBot.className = 'chat-msg chat-bot';
      msgBot.innerHTML = formatarRespostaIA(dados.resposta);
    }} else {{
      msgBot.className = 'chat-msg chat-erro';
      msgBot.textContent = dados.erro || 'Erro ao consultar a IA.';
    }}
    box.appendChild(msgBot);
  }} catch (e) {{
    const msgErro = document.createElement('div');
    msgErro.className = 'chat-msg chat-erro';
    msgErro.textContent = 'Não foi possível conectar ao servidor local.';
    box.appendChild(msgErro);
  }} finally {{
